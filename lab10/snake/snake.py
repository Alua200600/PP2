...
def get_or_create_user(conn, username):
    ...
    if result:
        ...
        print(f"Добро пожаловать, {username}! Съедено пищи в прошлую игру: {food_eaten}")
        return user_id, 0
    else:
        cur.execute("INSERT INTO \"user\" (username) VALUES (%s) RETURNING id", (username,))
        user_id = cur.fetchone()[0]
        conn.commit()
        print(f"Создан новый пользователь {username}. Начни игру!")
        return user_id, 0
...
def save_game_state(conn, user_id, food_eaten):
    ...
    conn.commit()
    print(f"Игра сохранена! Количество съеденной пищи: {food_eaten}")
...
while running:
    ...
    food_text = font.render(f"Eaten food: {food_eaten}", True, (255, 255, 255))
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_UP: direction = (0, -10)
                ...
                paused = not paused
                if paused:
                    print("Игра на паузе. Нажмите любую клавишу, чтобы продолжить.")
    ...
    if not paused:
        ...
        if head in snake[1:] or head[0] < 0 or head[0] >= 600 or head[1] < 0 or head[1] >= 400:
            print(f"Конец игры. Количество съеденной еды: {food_eaten}")
            save_game_state(conn, user_id, food_eaten)
            time.sleep(2)
            running = False
        ...
        if wall and pygame.Rect(*head, 10, 10).colliderect(pygame.Rect(*wall, 50, 10)):
            print(f"Конец игры (стена). Количество съеденной еды: {food_eaten}")
            save_game_state(conn, user_id, food_eaten)
            time.sleep(2)
            running = False
